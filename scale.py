...
try:
    ...
    print('ValueError: cannot floating your scale:', N)
    exit (0)    #print("P =", P)
...
if(P>=1):
    ...
    for ix in range (0, W):
        print(ix, '/', W-1)
        for iy in range(0, H):
            im2[iy][ix] = im1[iy*n][ix*n]
            #for i in range ((ix)*n+1,(ix+1)*n):
            #    for j in range ((iy)*n+1,(iy+1)*n):
            #        im2[iy][ix]+=im1[j][i]
           #     im2[iy][ix][0] /= 2 #=(im2[iy][ix][0] / n)
           #     im2[iy][ix][1] /= 2 #=(im2[iy][ix][1] / n)
           #     im2[iy][ix][2] /= 2 #=(im2[iy][ix][2] / n)
           # im2[iy][ix][0] /= 2 #=(im2[iy][ix][0] / n)
           # im2[iy][ix][1] /= 2 #=(im2[iy][ix][1] / n)
           # im2[iy][ix][2] /= 2 #=(im2[iy][ix][2] / n)
